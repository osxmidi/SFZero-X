...
filteregGain3(0.0f),filteregSlope3(0.0f), samplesUntilNextAmpSegmentfilter3(0), ampSegmentIsExponentialfilter3(false), streamer_(nullptr)
        // , midiVolume_(127)
{        
...
void sfzero::Voice::renderNextBlock(juce::AudioSampleBuffer &outputBuffer, int startSample, int numSamples)
{
  ...
  while (--numSamples >= 0)
  {
    ...
    if(streamer_){
      #ifdef SFZEROBUF
      if(region_->sample->CanStream() && buffer != streamer_->GetVoiceBuffer() && pos>= region_->sample->getpreBufferSize()){ 
      #else  
     ...
     // if(buffer == streamer_->GetVoiceBuffer() && pos>=streamer_->getNumSamplesFilled()-2){
        if(buffer == streamer_->GetVoiceBuffer() && pos>=streamer_->getNumSamplesFilled()){
        killNote();
 ...
 */
  ...
  if(region_!=nullptr){
    #ifdef SFZEROBUF
      if(region_->sample->CanStream() && sampleEnd_> region_->sample->getpreBufferSize()){
      if(streamer_==nullptr && sourceSamplePosition> region_->sample->getpreBufferSize()/2){
        streamer_=new SFZDiskStreamer("SFZStreamer", region_->sample->getFile(), formatManager_, region_->sample->getBuffer()->getNumChannels(), static_cast<int>(sampleEnd_), region_->sample->getpreBufferSize());
        streamer_ -> copyBuffer(buffer);
      }
      if(streamer_!=nullptr){
        streamer_->setCurrentSample(sourceSamplePosition, region_->sample->getpreBufferSize());
      }
