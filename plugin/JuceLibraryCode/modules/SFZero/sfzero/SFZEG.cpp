...
void sfzero::EG::startNote(const EGParameters *newParameters, int midiNoteNumber, float floatVelocity, double newSampleRate, float delaytime, const EGParameters *velMod)
{
  parameters_ = *newParameters;
  if (velMod)
  {
    parameters_.delay += floatVelocity * velMod->delay;
    parameters_.delay += delaytime;
